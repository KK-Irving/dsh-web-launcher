...
- **Multi-tab**: Ctrl+T to open new tabs, Ctrl+W to close, middle-click to close, drag-scroll for overflow
...
The built app includes auto-update support via GitHub Releases. To publish:
1. Tag a version: `git tag v1.0.0 && git push --tags`
2. Create a GitHub Release with the tag
3. Attach the built artifacts from `electron/dist/`
4. Running apps will detect the new release automatically
...
│   ├── preload/
│   │   └── index.js                 # Context bridge: safe IPC for both shell and content views
│   └── assets/
│       └── shell.html               # Tab bar UI (rendered in main window, BrowserViews below it)
└── dist/                            # Build output (git-ignored)
```

The app uses `BrowserView` for each tab — each tab is an independent web view loading the DSH Web UI. The main window only renders the tab bar (`shell.html`). This keeps tabs isolated and allows each to have its own navigation state.
...
| Global shortcut | ❌ | ✅ |
| Auto-update | ❌ | ✅ |
| Requires Node.js | ✅ | ✅ (bundled in build) |
| File size | ~50 KB scripts | ~180 MB packaged |
| Cross-platform potential | Windows only | Windows (macOS/Linux possible) |
